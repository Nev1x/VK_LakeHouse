...
## Схема `iceberg.silver.apartments_clean` (FROZEN, additive-only для 004/006/007)
...
| `listed_at` | timestamp | |
| `ceiling_height_m` | **decimal(4,2)** | лофт-маркер 007 (nullable); sanity 1.5–10.0 |
| `wall_material` | varchar | лофт-маркер 007 (nullable); источник без поля → NULL (lite) |
| `year_built` | bigint | лофт-маркер 007 (nullable); sanity 1800–2100 |
...
`silver_columns_version = 2` (в `schema_json` журнала; 007 — additive-эволюция схемы, см. ниже).

**Additive-эволюция схемы (007, expand-only I-6/I-10).** `silver_writer.ensure_table` работает по
образцу `bronze_writer`: `CREATE IF NOT EXISTS` для fresh + DESCRIBE-diff с `_SCHEMA` → `ALTER TABLE
ADD COLUMN` на каждую отсутствующую (nullable, идемпотентно). Без ALTER пополнение `_SCHEMA` дало бы
MERGE с INSERT на несуществующие колонки — падение записи ВСЕХ источников; поэтому эволюция — gate
любого расширения контракта. Существующие строки не переписываются (до reprocess новые колонки = NULL).
...
`_computed_at`, `_gold_run_id`. Медиана — **ТОЧНАЯ** (`array_agg(... ORDER BY ...)` → среднее двух
центральных элементов, `FILTER (WHERE price_rub IS NOT NULL)`) с явным `CAST(... AS DECIMAL(12,2))`;
детерминизм NFR-004. `price_per_m2` через `NULLIF(area_m2, 0)`.
...
(JSON as-is, passthrough для CV-фич 006 — gold не парсит); **лофт-маркеры 007** (passthrough из
silver, СТРОГО перед `is_loft`) `ceiling_height_m` DECIMAL(4,2), `wall_material` VARCHAR,
`year_built` BIGINT — все nullable; служебные `_silver_snapshot_id`,
`_source_transform_run_id`, `_gold_run_id` VARCHAR, `_computed_at` TIMESTAMP. Всего **26 колонок**
(23 базовых 006 + 3 маркера 007); `gold_columns_version = 2`.
...
- **is_loft always NULL** — target-заготовка (разметка «лофт/не-лофт» вне платформы, устав/006).
  ~~documented gap лофт-маркеров~~ **СНЯТ (007)**: сами маркеры
  `ceiling_height_m`/`wall_material`/`year_built` теперь доезжают до silver и features additive-
  колонками (v2) — см. раздел «Фича 007» ниже.
...
- **Медиана — точная** (`array_agg`+ORDER BY, fallback плана 004 T3). Прежняя `approx_percentile`
  снята: её недетерминизм на реальных данных владельца доказан 2026-07-22 (5 разных значений на 5
  идентичных прогонах по группе ~500 строк), что нарушало NFR-004 (тот же snapshot → тот же выход).
  Spike 004 ставил на демо-фикстурах (мелкие группы) и нестабильность не поймал.
...
| Медиана | `approx_percentile` (T3), fallback на точную «при необходимости» | **точная `array_agg`+ORDER BY** (fallback T3 активирован 2026-07-22) | spike ставил на мелких демо-фикстурах и не поймал недетерминизм approx_percentile; реальные данные владельца (6050 строк, группы 400–570) доказали его — нарушение NFR-004. Точная медиана детерминирована, демо-объём выдержал (build-gold в бюджете) |
...
- **Квирк `/api/plugins/<id>/settings.enabled`:** для datasource-плагинов Grafana отдаёт
  `enabled: false` (это app-toggle, не «плагин доступен») — наивная проверка дала бы ложный FAIL.
  Надёжный сигнал работоспособности: агрегатный `/api/plugins?embedded=0` (`enabled: true`) +
  datasource health == OK. Интеграционный тест НЕ полагается на `/settings.enabled`.
- **browser-QA (визуальный проход дашбордов renata/semiglazka) — documented gap** в текущем
  runtime: браузер-capability (Playwright MCP) не подключён. Покрытие обеспечено API/JSON-уровнем
  (структура/секреты/bounded-SQL unit-тестами), datasource health == OK и рендером данных через
  `/api/ds/query` по каждой панели. Визуальная проверка — при доступности браузер-capability.
...
# Export (фича 006 — версионированный ML-датасет, as-built)

CLI `loftnav export-dataset`: `gold.apartments_features` (пин на snapshot, ORDER BY id, чанками) →
parquet + jsonl + manifest.json в bucket `ml-datasets` под immutable-версией `datasets/vNNN/`.
Данные не покидают машину (I-1; «S3» = локальный MinIO). Замыкает пайплайн — воспроизводимый
датасет признаков для будущего обучения ML. Финальная фича бэклога.

## CLI / Make

| Команда | Действие |
|---|---|
| `make export-dataset` / `loftnav export-dataset [--format parquet\|jsonl\|both]` | экспорт версии |
| `make export-dataset-demo` | полная цепочка ingest → transform → build-gold → export |

## Раскладка `ml-datasets`

```
datasets/
  v001/
    data.parquet     # parquet (pyarrow, decimal нативно)
    data.jsonl       # jsonl (Decimal → СТРОКА, timestamp → ISO)
    manifest.json    # маркер валидности версии (пишется ПОСЛЕДНИМ)
  v002/ ...
```

Версия — `vNNN` (строгий `^v\d{3}$`); следующая = max(vNNN)+1 по `CommonPrefixes`
(`list_objects_v2(prefix='datasets/', delimiter='/')` с пагинацией IsTruncated/ContinuationToken —
полный скан, иначе занижение max→коллизия). Пустой bucket → v001. Мусор не по шаблону — лог+игнор.
Immutable (I-2/I-6): vNNN никогда не перезаписывается — `put_or_fail`/`upload_or_fail` **fail-loud**
на коллизию (НЕ idempotent-skip `put_if_absent` raw).

## Единый проход (bounded RAM, NFR-001)

Один read-loop: `SELECT <явные колонки> FROM apartments_features FOR VERSION AS OF <snapshot>
ORDER BY id`, `fetchmany(5000)`. На каждый чанк ОДНОВРЕМЕННО: `ParquetWriter.write_table`
(потоково, НЕ `df.to_parquet` — тот пишет целиком), jsonl-строки, инкремент sha256 обоих файлов.
Данные пишутся во временные файлы на диске → потоковая загрузка `upload_file` (не читаем датасет в
RAM). Пиковая память ≤ чанк. snapshot-пин переиспользует `marts.snapshots_relation` (не переписан).

## Манифест (frozen по I-6, FR-007)

`manifest_schema_version`, `dataset_version`, `created_at` (UTC ISO, **python-сторона**
`datetime.now(UTC)`, не Trino), `run_id`, `source_table`, `source_snapshot_id`,
`gold_columns_version`, `row_count`, `formats`, **`photo_handling: "links"`**,
**`target_populated: false`**, `schema` (колонки/типы из DESCRIBE + `null_count` для is_loft =
row_count), `files` (path/format/**sha256**/size_bytes — из инкремента записи, не повторный GET),
`loftnav_export_version`, `notes` (jsonl_decimal_as_string и т.п.). Валидный JSON.

**Честность (US-5):** `target_populated: false` — is_loft НЕ размечен (всегда NULL, разметка вне
платформы); `photo_handling: "links"` — фото не скачаны.

## Детерминизм (FR-008)

Пин snapshot + `ORDER BY id` → одинаковый снапшот features даёт одинаковый НАБОР и ПОРЯДОК строк.
Критерий воспроизводимости — идентичность **СОДЕРЖИМОГО** (sha256 jsonl-представления), НЕ
байт-в-байт parquet (parquet embed'ит writer-метаданные/тайминги). Два экспорта на одном snapshot →
разные vNNN, но идентичный jsonl-sha и один source_snapshot_id.

## Порядок записи (atomicity, FR-006)

Data-файлы пишутся ПЕРВЫМИ, `manifest.json` — ПОСЛЕДНИМ (= маркер валидности). Частичный сбой до
манифеста оставляет версию БЕЗ манифеста = невалидна («не полу-версия, притворяющаяся готовой»).
Потребитель обязан проверять наличие `manifest.json`. Journal `stage='export'` (try/finally,
content_hash=snapshot_id); общий pipeline-lock (с ingest/transform/build-gold, FR-011).

## I-4-трактовка (egress-зона, decision record)

I-4 «красная линия» — запрет ЧТЕНИЯ managed parquet/metadata таблиц medallion в обход Trino и
прямой записи в **`warehouse`**. 006 читает features ТОЛЬКО через Trino (FR-002) — read-path
compliant. Запись в `ml-datasets` — НЕ доступ к таблице каталога: bucket вне Iceberg-каталога, Trino
туда не пишет, это терминальная **egress-зона выгрузки**, структурно симметричная `raw`
(терминальный ingress вне каталога); разделение buckets установлено в 001. `S3Store` bucket —
параметр конструктора с allowlist `{raw, ml-datasets}` (hard-fail на `warehouse`/прочее —
defense-in-depth). **Known Risk:** рекомендован не-блокирующий PATCH устава 1.0.0→1.0.1, уточняющий
«единственное исключение» I-4 на две ingress/egress-зоны вне каталога (решение владельца, Часть III).

## Безопасность

- **Фото — ссылки, 0 исходящих HTTP** (FR-009/NFR-004): `photo_urls` passthrough as-is, НИКАКОГО
  скачивания (SSRF-риск: URL из непроверенных объявлений может указывать на внутренние `data_net`;
  + storage/legal). Проверяется тестом (monkeypatch `socket.connect` — assert-fail на любой
  не-localhost хост во время export).
- Данные не покидают машину (I-1); креды MinIO/Trino из env; 0 новых портов.
- Целостность: manifest + sha256 файлов = независимая проверка потребителем (I-13).

## Отклонения от плана 006 (as-built, обоснование I-13)

| Область | План | Факт | Причина |
|---|---|---|---|
| Загрузка в S3 | `put_or_fail(bytes)` | data-файлы через `upload_or_fail` (streaming `upload_file` с диска), manifest — `put_or_fail(bytes)` | bounded RAM (NFR-001): читать весь parquet/jsonl в память для put свело бы на нет потоковую запись; манифест мал — put байтами |
| pyarrow пин | «точная версия на реализации» | **25.0.0** (wheel py3.12, с pandas 3.0.3) | актуальная стабильная на момент реализации |

# Фича 007 — лофт-маркеры в silver и features (as-built)

Реальные данные владельца (6050 квартир) содержат ключевые признаки «лофтовости»
(`ceiling_height_m`, `wall_material`, `year_built`), которые 004 честно задокументировала как gap
(их не было в silver-модели). 007 — **additive**-расширение silver и features тремя nullable-
колонками + маппинги + bump версий контрактов (`silver_columns_version` 1→2, `gold_columns_version`
1→2). Разметка `is_loft` НЕ трогается (остаётся NULL; эвристики по маркерам запрещены как лже-таргет).

## Слои изменения (expand-only I-6/I-10)

- **silver** (`silver_writer`): `_SCHEMA`/`MAPPABLE_FIELDS` += 3 поля; `ensure_table` получила
  ALTER-эволюцию (DESCRIBE-diff → `ALTER TABLE ADD COLUMN`, идемпотентно) — **первоочередной gate**:
  без неё пополнение `_SCHEMA` сломало бы MERGE всех источников (INSERT на несуществующие колонки).
- **sanity** (`config.SANITY_DEFAULTS`): `ceiling_height_m` 1.5–10.0, `year_built` 1800–2100 (вне
  диапазона → quarantine, как price/area); `wall_material` — VARCHAR без диапазона.
- **маппинги**: `configs/mapping/{apartments,apartments_apartments}.toml` += 3 поля
  (`ceiling_height_m` cast=decimal, `wall_material` без cast, `year_built` cast=bigint);
  `apartments_lite.toml` += только `ceiling_height_m`+`year_built` (в его bronze-схеме нет
  `wall_material` → колонка NULL). Изменение конфига → штатный reprocess-протокол 003.
- **features/export**: `gold/features.py` += 3 passthrough-колонки СТРОГО перед `is_loft`;
  `export/schema.py FEATURES_COLUMNS` += те же тем же порядком (явный список, не `SELECT *`).
  Итог — **26 колонок** (23 базовых + 3 маркера).

## As-built проверки (реальные данные владельца, стек живой)

- silver 6050 (apartments 3000 / apartments_apartments 3000 / apartments_lite 50); маркеры
  заполнены у apartments/apartments_apartments (потолки 2.50–3.40, годы 1958–2026, 5 материалов
  стен), у lite `wall_material` = NULL (ceiling/year заполнены). 5 apartments-строк в quarantine —
  пустой `price_rub` (наследие данных, НЕ маркеры).
- features 6050 × 26; export новой версии (`v044`): `pandas.read_parquet` видит 26 колонок в порядке
  `FEATURES_COLUMNS`, `manifest.gold_columns_version = 2`, `schema` = 26 записей. Старые версии
  immutable — sha `manifest.json` v001 не изменился (gold_columns_version=1, 23 колонки).

## NFR-001 — замер полного цикла

**Wall-clock полного цикла (reprocess ×3 источника = 6050 строк → build-gold → export-dataset):
≈ 80.5 с (1.34 мин).** Бюджет NFR-001 — ≤ 5 мин (300 с); фактический запас ×3.7. Замер на живом
стеке (Trino 483 + Iceberg/MinIO, 4 контейнера healthy), одиночный прогон CLI. 0 новых зависимостей,
0 новых портов (NFR-003).

## Rollback (I-10)

Expand-only: новые колонки nullable, существующие строки не переписываются. Откат кода = `git
revert` (колонки остаются в таблицах NULL-сиротами — безвредны; Iceberg `DROP COLUMN` возможен
вручную, данные не теряются). Откат данных не требуется (reprocess переигрывает из полного bronze;
старые версии датасета immutable). Частичный сбой ALTER → идемпотентный повтор докатывает недостающие.
