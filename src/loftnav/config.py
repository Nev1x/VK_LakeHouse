...
# (0 = студия); этаж/этажность 1..200 (небоскрёбы); метро 0..240 мин; потолки 1.5..10.0 м
# (ниже — ошибка/подвал, выше — не жильё); год постройки 1800..2100. Переопределяемы через env.
# wall_material — VARCHAR без диапазона (sanity_ok пропускает поля без записи).
SANITY_DEFAULTS: dict[str, tuple[float, float]] = {
    ...
    "ceiling_height_m": (1.5, 10.0),        # 007: лофт-маркер (наблюдаемый диапазон 2.5–3.4)
    "year_built": (1800.0, 2100.0),         # 007: старый фонд..новостройки
}
