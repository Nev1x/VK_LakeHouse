...
class IngestConfig:
    ...
    max_field_bytes: int         # FR-002: cap длины одного поля / сериализованного JSON
    read_chunk_rows: int         # потоковое чтение (pandas chunksize)
    insert_chunk_rows: int       # строк в одном multi-row INSERT
    insert_chunk_bytes: int      # байтовый cap чанка (широкие строки)
    ...
    def from_env() -> IngestConfig:
        return IngestConfig(
            ...
            max_field_bytes=_int_env("LOFTNAV_MAX_FIELD_BYTES", 1_000_000),
            read_chunk_rows=_int_env("LOFTNAV_READ_CHUNK_ROWS", 5000),
            insert_chunk_rows=_int_env("LOFTNAV_INSERT_CHUNK_ROWS", 1000),
            insert_chunk_bytes=_int_env("LOFTNAV_INSERT_CHUNK_BYTES", 4_000_000),
